...
Umfangreiche Astronomie- & Astrologie-PWA, portrait-optimiert für das iPhone, komplett offline berechnet.
Helles, durchgängiges Design nach dem Vorbild von HimmelsWahr (kein dunkles Standarddesign mehr, Dunkelmodus bleibt optional wählbar).
...
- **Sternenhimmel**: ein eigenständig entwickelter, live berechneter Sternenhimmel (Technologie „SterneWahr") als eingebetteter Vollbild-Bereich – mit Zeitreise, Sternschnuppenströmen, Sternenlicht-Laufzeiten, Kompass-/Sensor-Ausrichtung, „Meine Geburtsnacht" und speicherbaren Erinnerungen; die Himmelsfläche selbst bleibt dunkel, damit Sterne sichtbar sind, die Bedienoberfläche folgt dem hellen AstroWahr-Design
...
## Technik
- Vanilla HTML/CSS/JS, keine Frameworks, keine externen Bibliotheken/CDNs, keine Google Fonts oder Roboto (nur Systemschriften)
...
- Der Sternenhimmel-Bereich (sternewahr-*.js/html) ist eine eigenständige, in AstroWahr eingebettete Berechnungs- und Darstellungs-Engine nach Jean Meeus/E. M. Standish/IMO (siehe Lizenzen), technisch unabhängig von astro.js
- Standort: für „Sonnenauf-/-untergang" und „Sternenhimmel" per Geolocation automatisch ermittelbar (mit Berechtigungsabfrage) oder weiterhin frei per Hand eingebbar; Geburtsorte in Horoskopen bleiben ausschließlich manuell
- Alle Daten ausschließlich lokal (localStorage, Präfixe „astrowahr." und, nur im eingebetteten Sternenhimmel, „zh:"), kein Server, kein Tracking
...
service-worker.js, manifest.json, impressum.html, datenschutz.html, lizenzen.html,
icon-180/192/512.png, sternewahr-index.html, sternewahr-style.css, sternewahr-astro.js, sternewahr-stars.js, sternewahr-render.js, sternewahr-app.js, README.md

## Rechtliches
Impressum, Datenschutzerklärung und Quellen/Lizenzen sind sowohl als eigene Seiten (impressum.html,
datenschutz.html, lizenzen.html) als auch als In-App-Tabs (Mehr → Rechtliches) erreichbar und
inhaltlich identisch. Der Datenschutztext ist gegen die tatsächlich verwendeten localStorage-Keys
abgeglichen (astrowahr.profiles, astrowahr.tarot.favorites, astrowahr.tarot.stats, astrowahr.theme,
astrowahr.onboarded, astrowahr.currentloc sowie die zh:-Schlüssel des eingebetteten Sternenhimmels);
das Backup (Einstellungen) exportiert/importiert Profile sowie Tarot-Favoriten/-Statistik. Alle
astrologischen und Tarot-Inhalte sind als Unterhaltung/Reflexionshilfe gekennzeichnet, nicht als
wissenschaftliche, medizinische, psychologische, rechtliche oder finanzielle Beratung; der
Astronomie-Bereich bildet den aktuellen Kenntnisstand vereinfacht ab (Sonnenzeiten und
Jahreszeiten-Zeitpunkte auf wenige Minuten genau, Mondzahlen der Gasriesen mit Stand 2026
gekennzeichnet, da sich diese durch neue Entdeckungen laufend ändern). Tarot-Kartentexte und -Bilder
sind eigene Erstellungen ohne Bezug zu bestehenden Decks (z. B. Rider-Waite-Smith). Die
Weltraumkunde-Inhalte sind eigenständig formulierte Fakten ohne Verwendung von Fotografien,
Missionslogos oder sonstigen Bildern Dritter (z. B. NASA/ESA) – ausschließlich selbst gestaltete,
vereinfachte Darstellungen. Standort, Bewegungssensoren und Kamera werden ausschließlich nach
ausdrücklicher Erlaubnis und rein lokal genutzt, nie übertragen. Keine externen Skripte/CDNs, keine
Analytics.

## Lizenz
© 2026 Jan Dierlich. Alle Rechte vorbehalten. Der Quellcode wird zu Informations- und
Referenzzwecken öffentlich einsehbar auf GitHub bereitgestellt, steht jedoch unter keiner
Open-Source-Lizenz (z. B. MIT). Eine Nutzung, Vervielfältigung, Veränderung oder Weiterverbreitung
– ganz oder in Teilen, auch von Code, Gestaltung, Texten, Tarot-Kartenkunst oder Icons – ist ohne
vorherige schriftliche Zustimmung des Urhebers nicht gestattet. Siehe auch [lizenzen.html](lizenzen.html)
und [LICENSE](LICENSE).
